...
运行时测试和 benchmark 需要本地 CUDA GPU。托管 CI 主要负责格式检查、文档站点检查，以及 Pages 可构建性。
...
| 环境 | 能证明什么 |
|------|------------|
| 托管 CI | 格式检查、文档结构、路由完整性、Pages 可构建性 |
| 本地 CUDA GPU | CMake 构建有效性、运行时正确性、fallback 行为、benchmark 性能 |
