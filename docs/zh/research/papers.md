...
共享内存 bank 冲突消除的实践方法。[消除 Bank Conflict 内核](/zh/academy/kernel-bank-free)中的填充策略遵循此方法。
...
虽然聚焦于归约操作，但此白皮书引入了双缓冲概念，用于重叠内存传输与计算。[双缓冲内核](/zh/academy/kernel-double-buffer)将其应用于 GEMM 的瓦片加载-计算周期。
...
Volta Tensor Core 的微基准测试研究。有助于理解 [Tensor Core WMMA 内核](/zh/academy/kernel-tensor-core)背后的实际吞吐量和延迟特征。
...
原始 Roofline 模型论文。提供了讨论算术强度、内存带宽限制和计算上限的词汇，这是 [Benchmark 纪律](/zh/academy/benchmark-discipline)的基础。
...
- [参考资料清单](/zh/research/references) — 文档、工具和代码库的完整目录
- [延伸阅读路线](/zh/research/further-reading) — 有观点的学习路径
- [资源中心](/zh/research/) — 基于场景的入口点
