...
- 先回看 [分块内核](/zh/kernel-tiled) 与 [Memory Flow](/zh/architecture/memory-flow)，把本仓库的具体例子放回脑中。
- 再读 [Anatomy of High-Performance Matrix Multiplication](/zh/references#论文与性能心智模型)，理解 blocking 与数据复用为什么是系统级设计问题。
- 最后对照 [CUTLASS](/zh/references#示范仓库与生产级样例)，观察生产级 CUDA 库如何表达分块层级与任务分工。
...
- 先看 [双缓冲](/zh/kernel-double-buffer) 与 [消除 Bank Conflict](/zh/kernel-bank-free)，因为寄存器压力与 shared memory 布局通常就是 occupancy 变化的来源。
- 再配合 [CUDA Occupancy Calculator 与 Nsight Compute 资料](/zh/references#profiler、工具与诊断资料)，把 launch 配置和 active warps、寄存器、shared memory 限制对应起来。
- 然后回到 [诊断闭环](/zh/methodology/diagnosis-loop)，把 occupancy 变成一个待验证假设，而不是万能答案。
...
- 先用 [Benchmark 纪律](/zh/methodology/benchmark-discipline) 与 [Benchmark 范围](/zh/validation/benchmark-scope) 确保你手上的数字本身值得解释。
- 再把 [Nsight Compute 中与 roofline 相关的资料](/zh/references#profiler、工具与诊断资料) 和 [性能模型类阅读](/zh/references#论文与性能心智模型) 对照起来看。
...
- 先读 [Tensor Core 路径](/zh/architecture/tensor-core-path) 与 [Tensor Core WMMA](/zh/kernel-tensor-core)。
- 再打开 [WMMA API 与混合精度指南](/zh/references#cuda-与-nvidia-官方文档)，确认 fragment 大小、对齐要求、数据转换成本。
...
- 从 [诊断闭环](/zh/methodology/diagnosis-loop) 开始。
- 配合 [Nsight Systems、Nsight Compute 与 Compute Sanitizer](/zh/references#profiler、工具与诊断资料)，先拆开 launch、内存与正确性问题。
