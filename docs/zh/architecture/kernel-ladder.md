...
|------|----------------|----------|-----------|--------|
| Naïve | 全局内存重复访问、复用差 | 一个线程负责一个输出元素 | 可读，但明显受内存限制 | [朴素内核](/zh/academy/kernel-naive) |
| Tiled | 全局内存带宽压力 | 用共享内存分块并在 block 内复用 | 需要 barrier 与 tile-size 权衡 | [分块内核](/zh/academy/kernel-tiled) |
| Bank-Free | 共享内存争用 | 通过 padding 消除 bank 冲突模式 | 共享内存占用略增 | [消除 Bank Conflict](/zh/academy/kernel-bank-free) |
| Double Buffer | 加载与计算串行化 | 用 ping-pong 缓冲区重叠加载与计算 | 共享内存更多，调度更复杂 | [双缓冲](/zh/academy/kernel-double-buffer) |
| Tensor Core | CUDA Core 吞吐上限 | 在 Tensor Core 上执行 warp 级 WMMA | 需要 FP16 staging、shape guard 和 fallback 策略 | [Tensor Core WMMA](/zh/academy/kernel-tensor-core) |
