...
- Revisit [Tiled Kernel](/en/kernel-tiled) and [Memory Flow](/en/architecture/memory-flow) first so the project-specific examples are fresh.
- Then read [Anatomy of High-Performance Matrix Multiplication](/en/references#papers-and-performance-mental-models) to see why blocking and data reuse define the problem at a systems level.
- Compare that model with [CUTLASS](/en/references#exemplary-codebases-and-production-grade-samples) to see how a production CUDA library expresses hierarchy and tiling.
...
- Start with [Double Buffer](/en/kernel-double-buffer) and [Bank Conflict Free](/en/kernel-bank-free), because register pressure and shared-memory layout are the usual reasons occupancy changes.
- Use the [CUDA Occupancy Calculator and Nsight Compute references](/en/references#profiler-tooling-and-diagnosis-references) to connect launch configuration choices to active warps, shared-memory usage, and register limits.
- Then reread [Diagnosis Loop](/en/methodology/diagnosis-loop) so occupancy becomes one hypothesis to test, not the answer to every performance question.
...
- Use [Benchmark Discipline](/en/methodology/benchmark-discipline) and [Benchmark Scope](/en/validation/benchmark-scope) to make sure the reported number is even worth interpreting.
- Open the [Nsight Compute roofline-oriented documentation](/en/references#profiler-tooling-and-diagnosis-references) alongside [foundational performance-model reading](/en/references#papers-and-performance-mental-models).
...
- Read [Tensor Core Path](/en/architecture/tensor-core-path) and [Tensor Core WMMA](/en/kernel-tensor-core).
- Then open the [WMMA API reference and mixed-precision guidance](/en/references#official-cuda-and-nvidia-docs) to confirm fragment sizes, alignment requirements, and conversion costs.
...
- Start with [Diagnosis Loop](/en/methodology/diagnosis-loop).
- Use [Nsight Systems, Nsight Compute, and Compute Sanitizer references](/en/references#profiler-tooling-and-diagnosis-references) to separate launch issues, memory issues, and correctness issues.
