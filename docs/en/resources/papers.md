...
Practical treatment of bank conflict avoidance in shared memory. The padding strategy in [Bank-Free Kernel](/en/kernel-bank-free) follows this approach.
...
While focused on reduction, this whitepaper introduces the double-buffer concept for overlapping memory transfers with computation. The [Double Buffer Kernel](/en/kernel-double-buffer) applies this to GEMM's tile load-compute cycle.
...
Microbenchmarking study of Volta Tensor Cores. Useful for understanding the actual throughput and latency characteristics behind the [Tensor Core WMMA Kernel](/en/kernel-tensor-core).
...
The original roofline model paper. Provides the vocabulary for discussing arithmetic intensity, memory bandwidth limits, and compute ceilings that underpins [Benchmark Discipline](/en/methodology/benchmark-discipline).
...
- [Curated References](/en/references) — Full catalog of documentation, tools, and codebases
- [Further Reading Routes](/en/resources/further-reading) — Opinionated study paths
- [Resources Hub](/en/resources/) — Scenario-based entry points
