...
/**
 * Naive SGEMM with alpha and beta scaling
 * C = alpha * A * B + beta * C
 */
__global__ void naive_sgemm_kernel_scaled(const float *__restrict__ A,
                                          const float *__restrict__ B,
...
template <int BLOCK_SIZE = 32>
void launch_naive_sgemm_scaled(const float *A, const float *B, float *C, int M,
  ...
  naive_sgemm_kernel_scaled<<<gridDim, blockDim, 0, stream>>>(A, B, C, M, K, N,
                                                              alpha, beta);
